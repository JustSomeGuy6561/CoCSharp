...
namespace CoC.BodyParts
{
	internal class Feet : SimpleBodyPart<FootType>
	{
		...
		public static Feet GenerateDefault(FootType type)
		{
			return new Feet(type);
		}

		public bool UpdateType(FootType newType)
		{
			if (type == newType)
			{
				return false;
	...
	internal partial class FootType : SimpleBodyPartType
	{
		private static int indexMaker = 0;

		protected enum FootStyle { FEET, PAWS, HOOVES, INSECTOID, CLAWS, OTHER }
		...
		protected FootType(FootStyle style, SimpleDescriptor shortDesc, DescriptorWithArg<Feet> fullDesc) : base(shortDesc)
		{
			_index = indexMaker++;
			footStyle = style;
