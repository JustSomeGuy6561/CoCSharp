...
wet cloth - restored
	...
	[DataContract]
	internal class Body : PiercableBodyPart<Body, BodyType, NavelPiercings>, ISerializable
	{
		...
		protected Body(BodyType bodyType) : this(bodyType, null, null, null, null, HairFurColors.BLACK, null, null, null, null) { }
		public Body(BodyType bodyType, Arms arms, Back back, Ears ears, Face face, HairFurColors hairColor, LowerBody lowerBody, Neck neck, Tail tail, Wings wings)
		{
			...
			this.arms = arms ?? Arms.GenerateDefault();
			this.back = back ?? Back.GenerateDefault();
			this.ears = ears ?? Ears.GenerateDefault();
			this.face = face ?? Face.GenerateDefault();
			this.lowerBody = lowerBody ?? LowerBody.GenerateDefaultOfType(LowerBodyType.HUMAN);
			this.neck = neck ?? Neck.Generate();
			this.tail = tail ?? Tail.GenerateDefault();
			this.wings = wings ?? Wings.Generate();
			type = bodyType;
		...
		public bool UpdateBody(CockatriceBodyType cockatriceBodyType)
		{
			...
			if (_primaryEpidermis.fur.isNoFur())
			{
				...
				{
					updatePrimaryEpidermis(FurColor.Generate(hairColor));
				}
				else
				{
		...
		public bool UpdateBody(FurBodyType furryType, FurColor primaryColor, FurColor secondaryColor)
		{
			...
			{
				_secondaryEpidermis.Reset(furryType.epidermisType);
			}
		...
		public bool UpdateBody(ToneBodyType toneType, Tones primaryColor, Tones secondaryColor)
		{
			...
			{
				_secondaryEpidermis.Reset(toneType.epidermisType);
			}
		...
		{
			return arms.UpdateArmsFromBody(armType, _primaryEpidermis, _secondaryEpidermis, this.hairColor, this.type);
		}
		...
		public bool UpdateArmsAndDisplayMessage(ArmType armType, Player player)
		{
			return arms.UpdateArmsFromBodyAndDisplayMessage(armType, _primaryEpidermis, _secondaryEpidermis, this.hairColor, this.type, player);
		}

		public bool RestoreArmsAndDisplayMessage(Player player)
		{
			return arms.RestoreAndDisplayMessage(player);
		}
		#endregion

#warning add Tails
#warning add LowerBody
#warning add wings helper.
#warning add neck (ND)
#warning add horns (ND)
#warning add face (ND)
		...
		{
			toneEventArg.primaryTone = _primaryEpidermis.tone;
			toneEventArg.secondaryTone = _secondaryEpidermis.tone;
			toneEventArg.primaryToneActive = type.epidermisType.usesTone;
			OnToneAware(toneEventArg);
		}
		private void FurChanged()
		{
			furEventArg.primaryColor.UpdateFurColor(_primaryEpidermis.fur);
			furEventArg.secondaryColor.UpdateFurColor(_secondaryEpidermis.fur);
			OnFurAware(furEventArg);
		}

		private void SetupAndValidateData()
		{
		...
		private void updatePrimaryEpidermis(Tones tone)
		{
			_primaryEpidermis.ChangeTone(tone, true);
		}

		private void updatePrimaryEpidermis(FurColor fur)
		{
			_primaryEpidermis.ChangeFur(fur);
		}

		private void updateSecondaryEpidermis(Tones tone)
		{
			_secondaryEpidermis.ChangeTone(tone, true);
		}

		private void updateSecondaryEpidermis(FurColor fur)
		{
			_secondaryEpidermis.ChangeFur(fur, true);
		}
		#endregion
		...
		protected Body(SerializationInfo info, StreamingContext context)
		{
			...
			arms = (Arms)info.GetValue(nameof(arms), typeof(Arms));
			back = (Back)info.GetValue(nameof(back), typeof(Back));
			ears = (Ears)info.GetValue(nameof(ears), typeof(Ears));
			face = (Face)info.GetValue(nameof(face), typeof(Face));
			lowerBody = (LowerBody)info.GetValue(nameof(lowerBody), typeof(LowerBody));
			neck = (Neck)info.GetValue(nameof(neck), typeof(Neck));
			tail = (Tail)info.GetValue(nameof(tail), typeof(Tail));
			SetupAndValidateData();
		}
		...
		public void GetObjectData(SerializationInfo info, StreamingContext context)
		{
			...
			info.AddValue(nameof(arms), arms, typeof(Arms));
			info.AddValue(nameof(back), back, typeof(Back));
			info.AddValue(nameof(ears), ears, typeof(Ears));
			info.AddValue(nameof(face), face, typeof(Face));
			info.AddValue(nameof(lowerBody), lowerBody, typeof(LowerBody));
			info.AddValue(nameof(neck), neck, typeof(Neck));
			info.AddValue(nameof(tail), tail, typeof(Tail));
		}
		#endregion
	}
