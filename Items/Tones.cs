...
namespace CoC.Items
{
	...
	public class Tones : EpidermalColors
	{
		...
		protected Tones(Color rgb, string colorName) : base(rgb, colorName)
		{
			availableTones.Add(this);
		}

		protected Tones(Color rgb) : base(rgb)
		{
			availableTones.Add(this);
		}
		...
		{
			if (currentHairFur != HairFurColors.RAINBOW)
			{
				return NearestTone(currentHairFur);
			}
			//otherwise just make it random. fuck it.
