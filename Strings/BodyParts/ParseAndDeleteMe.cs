	...
	{	
		...
		if (player.face.type == Face.HUMAN || player.face.type == Face.SHARK_TEETH || player.face.type == Face.BUNNY || player.face.type == Face.SPIDER_FANGS || player.face.type == Face.FERRET_MASK)
		{
			if (player.hasPlainSkin() || player.hasGooSkin())
				Console.WriteLine("  Your face is human in shape and structure, with [skin].");
			if (player.hasFur())
				Console.WriteLine("  Under your [skinFurScales] you have a human-shaped head with [skin.noadj].");
			if (player.hasScales())
				Console.WriteLine("  Your face is fairly human in shape, but is covered in [skin].");
			if (player.face.type == Face.SHARK_TEETH)
				Console.WriteLine("  A set of razor-sharp, retractable shark-teeth fill your mouth and gives your visage a slightly angular appearance.");
			else if (player.face.type == Face.BUNNY)
				Console.WriteLine("  The constant twitches of your nose and the length of your incisors gives your visage a hint of bunny-like cuteness.");
			else if (player.face.type == Face.SPIDER_FANGS)
				Console.WriteLine("  A set of retractable, needle-like fangs sit in place of your canines and are ready to dispense their venom.");
			else if (player.face.type == Face.FERRET_MASK)
				Console.WriteLine("  The [skinFurScales] around your eyes is significantly darker than the rest of your face, giving you a cute little ferret mask.");
		}
		...
		else if (player.face.type == Face.RACCOON_MASK)
		{
			...
			if (!player.isFurryOrScaley())
			{
				...
				if ((player.skin.tone == "ebony" || player.skin.tone == "black") && (player.hasPlainSkin() || player.hasGooSkin()))
					Console.WriteLine(", though with your dusky hue, the black raccoon mask you sport isn't properly visible.");
				else Console.WriteLine(", though it is decorated with a sly-looking raccoon mask over your eyes.");
			}
			...
			else
			{
				...
				if (((player.skin.furColor == "black" || player.skin.furColor == "midnight") && player.isFurryOrScaley()))
					Console.WriteLine("  Under your [skinFurScales] hides a black raccoon mask, barely visible due to your inky hue, and");
				else Console.WriteLine("  Your [skinFurScales] are decorated with a sly-looking raccoon mask, and under them");
				Console.WriteLine(" you have a human-shaped head with [skin.noadj].");
			}
		}
		else if (player.face.type == Face.RACCOON)
		{
			Console.WriteLine("  You have a triangular raccoon face, replete with sensitive whiskers and a little black nose; a mask shades the space around your eyes, set apart from your [skinFurScales] by a band of white.");
			//(if skin)
			if (player.hasPlainSkin())
				Console.WriteLine("  It looks a bit strange with only the skin and no fur.");
			else if (player.hasReptileScales())
				Console.WriteLine("  The presence of said scales gives your visage an eerie look, more reptile than mammal.");
		}
		else if (player.face.type == Face.FOX)
		{
			Console.WriteLine("  You have a tapered, shrewd-looking vulpine face with a speckling of downward-curved whiskers just behind the nose.");
			if (player.hasPlainSkin())
				Console.WriteLine("  Oddly enough, there's no fur on your animalistic muzzle, just [skinFurScales].");
			else if (player.hasFur())
				if (player.hasDifferentUnderBody())
					Console.WriteLine("  Your muzzle is decorated with [skinFurScales] on your upper jaw and head and with [underBody.skinFurScales] on your lower jaw.");
				else
					Console.WriteLine("  A coat of [skinFurScales] decorates your muzzle.");
			else if (player.hasScales())
				Console.WriteLine("  Strangely, [skinFurScales] adorn every inch of your animalistic visage.");
		}
		else if (player.face.type == Face.BUCKTEETH)
		{
			//appearance
			Console.WriteLine("  Your face is generally human in shape and structure, with [skin]"
					  + (player.hasFur() ? " under your [skinFurScales]" : "") + " and mousey buckteeth.");
		}
		else if (player.face.type == Face.MOUSE)
		{
			//appearance
			Console.WriteLine("  You have a snubby, tapered mouse's face, with whiskers, a little pink nose, and [skin]"
		...
		{
			if (player.hasPlainSkin() || player.hasGooSkin())
				Console.WriteLine("  You have a fairly normal face, with [skin].  The only oddity is your pair of dripping fangs which often hang over your lower lip.");
			if (player.hasFur())
				Console.WriteLine("  Under your [skinFurScales] you have a human-shaped head with [skin.noadj].  In addition, a pair of fangs hang over your lower lip, dripping with venom.");
			if (player.hasScales())
				Console.WriteLine("  Your face is fairly human in shape, but is covered in [skinFurScales].  In addition, a pair of fangs hang over your lower lip, dripping with venom.");
		}
		//horse-face
		if (player.face.type == Face.HORSE)
		{
			if (player.hasPlainSkin() || player.hasGooSkin())
				Console.WriteLine("  Your face is equine in shape and structure.  The odd visage is hairless and covered with [skinFurScales].");
			if (player.hasFur())
				Console.WriteLine("  Your face is almost entirely equine in appearance, even having [skinFurScales].  Underneath the fur, you believe you have [skin.noadj].");
			if (player.hasScales())
				Console.WriteLine("  You have the face and head structure of a horse, overlaid with glittering [skinFurScales].");
		}
		//wolf-face
		if (player.face.type == Face.WOLF)
		{
			Console.WriteLine("  You have an angular wolf's face complete with a muzzle and black nose");
			if (!player.hasFur())
				Console.WriteLine(", though devoid of any fur.");
			else
				Console.WriteLine(".");
		}
		//dog-face
		...
		{
			if (player.hasPlainSkin() || player.hasGooSkin())
				Console.WriteLine("  You have a dog-like face, complete with a wet nose.  The odd visage is hairless and covered with [skinFurScales].");
			if (player.hasFur())
				Console.WriteLine("  You have a dog's face, complete with wet nose and panting tongue.  You've got [skinFurScales], hiding your [skin.noadj] underneath your furry visage.");
			if (player.hasScales())
				Console.WriteLine("  You have the facial structure of a dog, wet nose and all, but overlaid with glittering [skinFurScales].");
		}
		//cat-face
		if (player.hasCatFace())
		{
			...
			{
				if (player.hasDifferentUnderBody())
					Console.WriteLine("  You have a cat-like face, complete with moist nose and whiskers."
			...
			{
				Console.WriteLine("  Your face is human in appearance. You have a set of sharp cat-like teeth in your mouth.");
			}
		}
		//Minotaaaauuuur-face
		...
		{
			if (player.hasPlainSkin() || player.hasGooSkin())
				Console.WriteLine("  You have a face resembling that of a minotaur, with cow-like features, particularly a squared off wet nose.  Despite your lack of fur elsewhere, your visage does have a short layer of [furColor] fuzz.");
			if (player.hasFur())
				Console.WriteLine("  You have a face resembling that of a minotaur, with cow-like features, particularly a squared off wet nose.  Your [skinFurScales] thickens noticeably on your head, looking shaggy and more than a little monstrous once laid over your visage.");
			if (player.hasScales())
				Console.WriteLine("  Your face resembles a minotaur's, though strangely it is covered in shimmering scales, right up to the flat cow-like nose that protrudes from your face.");
		}
		//Lizard-face
		if (player.face.type == Face.LIZARD)
		{
			if (player.hasPlainSkin() || player.hasGooSkin())
				Console.WriteLine("  You have a face resembling that of a lizard, and with your toothy maw, you have quite a fearsome visage.  The reptilian visage does look a little odd with just [skin].");
			if (player.hasFur())
				Console.WriteLine("  You have a face resembling that of a lizard.  Between the toothy maw, pointed snout, and the layer of [skinFurScales] covering your face, you have quite the fearsome visage.");
			if (player.hasScales())
			{
				Console.WriteLine("  Your face is that of a lizard, complete with a toothy maw and pointed snout.");
				if (!player.hasReptileUnderBody())
					Console.WriteLine("  Reflective [skinFurScales] complete the look, making you look quite fearsome.");
				else
					Console.WriteLine("  Reflective [skinFurScales] on your upper jaw and head and [underBody.skinFurScales] on your lower jaw complete the look, making you look quite fearsome.");
			}
		}
		if (player.face.type == Face.DRAGON)
		{
			Console.WriteLine("  Your face is a narrow, reptilian muzzle.  It looks like a predatory lizard's, at first glance, but with an unusual array of spikes along the under-jaw.  It gives you a regal but fierce visage.  Opening your mouth reveals several rows of dagger-like sharp teeth.  The fearsome visage is decorated by [skinFurScales]");
			Console.WriteLine(player.hasReptileUnderBody() ? " on your upper jaw and head and [underBody.skinFurScales] on your lower jaw." : ".");
		}
		if (player.face.type == Face.KANGAROO)
		{
			Console.WriteLine("  Your face is ");
			if (player.hasPlainSkin())
				Console.WriteLine("bald");
			else Console.WriteLine("covered with [skinFurScales]");
			Console.WriteLine(" and shaped like that of a kangaroo, somewhat rabbit-like except for the extreme length of your odd visage.");
		}
		//<mod>
		if (player.face.type == Face.PIG)
		{
			Console.WriteLine("  Your face is like that of a pig, with [skinTone] skin, complete with a snout that is always wiggling.");
		}
		if (player.face.type == Face.BOAR)
		{
			Console.WriteLine("  Your face is like that of a boar, ");
			if (player.hasFur())
				Console.WriteLine("with [skinTone] skin underneath your [furColor] fur");
			Console.WriteLine(", complete with tusks and a snout that is always wiggling.");
		}
		if (player.face.type == Face.RHINO)
		{
			Console.WriteLine("  Your face is like that of a rhino");
			if (player.hasPlainSkin())
				Console.WriteLine(", with [skin], complete with a long muzzle and a horn on your nose.");
			else
				Console.WriteLine(" with a long muzzle and a horn on your nose.  Oddly, your face is also covered in [skinFurScales].");
		}
		if (player.face.type == Face.ECHIDNA)
		{
			Console.WriteLine("  Your odd visage consists of a long, thin echidna snout.");
			if (player.hasPlainSkin())
				Console.WriteLine("  The [skin] that is revealed by your lack of fur looks quite unusual.");
			else if (player.hasFur())
				Console.WriteLine("  It's covered in [skinFurScales].");
			else if (player.hasScales())
				Console.WriteLine("  It's covered in [skinFurScales], making your face even more unusual.");
		}
		if (player.face.type == Face.DEER)
		{
			Console.WriteLine("  Your face is like that of a deer, with a nose at the end of your muzzle.");
			if (player.hasPlainSkin())
				Console.WriteLine("  The [skin] that is revealed by your lack of fur looks quite unusual.");
			else if (player.hasFur())
				if (player.hasDifferentUnderBody())
					Console.WriteLine("  It's covered in [skinFurScales] on your upper jaw and head and [underBody.skinFurScales]"
							  + " on your lower jaw that covers your [skin.noadj] underneath.");
				else
					Console.WriteLine("  It's covered in [skinFurScales] that covers your [skin.noadj] underneath.");
			else if (player.hasScales())
				Console.WriteLine("  It's covered in [skinFurScales], making your face looks more unusual.");
		}
		...
		{
			Console.WriteLine("  Your face has a distinctive animalistic muzzle, proper from a red-panda, complete with a cute pink nose."
