...
namespace CoC.Backend.Save
{
	...
	public static class SaveSystem
	{
		...
		public static Type[] KnownTypeMagicTesting()
		{
			HashSet<Type> data = new HashSet<Type>();
			const BindingFlags bindingFlags = BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance;
			IEnumerable<Type> types = (globalData.Keys).Union(sessionData.Keys);
			foreach (var member in types)
			{
				var props = member.GetProperties(bindingFlags);
				foreach (var prop in props)
				{
					data.Add(prop.PropertyType);
				}
				var fields = member.GetFields(bindingFlags);
				foreach (var field in fields)
				{
					data.Add(field.FieldType);
				}
			}
			return data.ToArray();
		}
