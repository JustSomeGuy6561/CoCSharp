...
namespace CoC.Backend.BodyParts
{
	public partial class Genitals
	{
		...
		internal float CockGrowthMultiplier
		{
			...
			set => CheckChanged(ref _CockGrowthMultiplier, value, () => _cocks.ForEach(x=>x.cockGrowthMultiplier = value));
		}
		private float _CockGrowthMultiplier = 1;
		internal float CockShrinkMultiplier
		{
			get => _CockShrinkMultiplier;
			set => CheckChanged(ref _CockShrinkMultiplier, value, () => _cocks.ForEach(x=>x.cockShrinkMultiplier = value));
		}
		...
		{
			return new CockPerkHelper(CockGrowthMultiplier, CockShrinkMultiplier, NewCockDefaultSize, NewCockSizeDelta, MinCockLength);
		}
		#endregion
		#region Vaginas
		...
		internal bool unlockedDickNipples
		{
			...
			set => CheckChanged(ref _unlockedDickNipples, value, () => _breasts.ForEach(x=>x.nipples.unlockedDickNipples = value));
		}
